...
function Purchase() {
  ...
  const handleQuantitySubmit = async (e) => {
    if (e.key === 'Enter' && item && quantity) {
      ...
      try {
        await axios.post('http://localhost:3001/purchase/add', {
          date,
          items: [newItem]
        });
        setItem('');
        setQuantity('');
        console.log('Purchase added successfully:', newItem);
    ...
    <Container>
      <Navbar>
        <StyledUl>
          ...
          <StyledLi
            active={location.pathname === '/purchasedata'}
            onClick={() => handleNavigation('purchasedata')}
          >
            Data
          </StyledLi>
