...
function ReturnStock() {
  ...
  const handleQuantitySubmit = async (e) => {
    if (e.key === 'Enter' && item && quantity) {
      ...
      const returnedItem = { item, quantity: parsedQuantity };
      try {
        await axios.post('http://localhost:3001/return/add', {
          date,
          items: [returnedItem]
        });
        setItem('');
        setQuantity('');
        console.log('Return added successfully:', returnedItem);
    ...
    <Container>
      <Navbar>
        <StyledUl>
          ...
          <StyledLi
            active={location.pathname === '/returndata'}
            onClick={() => handleNavigation('returndata')}
          >
            Data
          </StyledLi>
