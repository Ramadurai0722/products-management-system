...
function Sales() {
  ...
  const handleQuantitySubmit = async (e) => {
    if (e.key === 'Enter' && item && quantity) {
      ...
      try {
        await axios.post('http://localhost:3001/sales/add', {
          date,
          items: [newItem]
        });
        setItem('');
        setQuantity('');
        console.log('Sale added successfully:', newItem);
    ...
    <Container>
      <Navbar>
        <StyledUl>
          ...
          <StyledLi
            active={location.pathname === '/saledata'}
            onClick={() => handleNavigation('saledata')}
          >
            Data
          </StyledLi>
